            ...
            <table class="menu-container" border="0">
                ...
                <tr class="menu-row">
                    <td class="menu-btn menu-icon-appoinment">
                        <a href="delete_mamagement.php" class="non-style-link-menu"><div><p class="menu-text">Delete Management</p></a></div>
                    </td>
                </tr>
                <tr class="menu-row" >
                ...
                <tr>
                    <td colspan="4" style="padding-top:0px;width: 100%;" >
                        ...
                        <form action="" method="post">
                            ...
                            <input type="date" name="date" class="input-text" id="sessionDate" min="<?php echo date('Y-m-d'); ?>" required><br>
...
<script>
    ...
    document.getElementById("sessionDate").setAttribute("min", today);
                        ...
                        <select name="docid" id="" class="box filter-container-items" style="width:90% ;height: 37px;margin: 0;" >
                            ...
                            <?php 
                                ...
                                $list11 = $database->query("select  * from  doctor order by dfname asc;");

                                for ($y=0;$y<$list11->num_rows;$y++){
                                    $row00=$list11->fetch_assoc();
                                    $sn=$row00["dfname"] . ' ' . $row00["dlname"];
                ...
                <?php
                    if($_POST){
                        ...
                        if(!empty($_POST["docid"])){
                            $docid=$_POST["docid"];
                            $sqlpt2=" doctor.doctor_id=$docid ";
                        }
                        //echo $sqlpt2;
                        //echo $sqlpt1;
                        $sqlmain= "select schedule.scheduleid,schedule.title,CONCAT(doctor.dfname, ' ', doctor.dlname) as docname,schedule.scheduledate,schedule.scheduletime,schedule.nop from schedule inner join doctor on schedule.docid=doctor.doctor_id ";
                        ...
                        $sqlmain= "select schedule.scheduleid,schedule.title,CONCAT(doctor.dfname, ' ', doctor.dlname) as docname,schedule.scheduledate,schedule.scheduletime,schedule.nop from schedule inner join doctor on schedule.docid=doctor.doctor_id  order by schedule.scheduledate desc";
    ...
    if($_GET){
        ...
        if($action=='add-session'){
            ...
            $sqlmain= "select schedule.scheduleid,schedule.title,CONCAT(doctor.dfname, ' ', doctor.dlname) as docname,schedule.scheduledate,schedule.scheduletime,schedule.nop from schedule inner join doctor on schedule.docid=doctor.doctor_id  where  schedule.scheduleid=$id";
            $result= $database->query($sqlmain);
            $row=$result->fetch_assoc();
            $docname=$row["docname"];
            $scheduleid=$row["scheduleid"];
            ...
            $sqlmain12= "select * from appointment inner join patient on patient.patient_id=appointment.pid inner join schedule on schedule.scheduleid=appointment.scheduleid where schedule.scheduleid=$id;";
